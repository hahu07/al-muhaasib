...
import { expenseService, expenseCategoryService, budgetService } from './src/services';
...
async function testValidationHooks() {
  ...
  console.log('   at the Juno satellite level BEFORE data reaches blockchain.\n');
  ...
  try {
    ...
    console.log('   🛡️ Validation hook prevented invalid data from reaching blockchain\n');
  }
  ...
  try {
    ...
    console.log(`   ✅ VALIDATION WORKING: ${error}`);
    console.log('   🛡️ Budget codes must be in XXX-000 format (e.g., ADM-001)\n');
  }
  ...
  try {
    ...
    console.log(`   ✅ SUCCESS: Created custom category "${customCategory.name}"`);
  ...
  console.log('The Al-Muhaasib system is protected by bulletproof data validation! 🛡️✨');
}
...
function simulateValidationScenarios() {
  ...
  const validationRules = [
    ...
    },
    {
      ...
      error: 'Invalid payment method. Must be one of: cash, bank_transfer, cheque, pos'
    },
    {
