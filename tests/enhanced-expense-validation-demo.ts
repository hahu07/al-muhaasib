...
async function demonstrateEnhancedExpenseValidation() {
  ...
  try {
    ...
    const utilitiesCategory = categories.find(c => c.category === 'utilities')!;
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Self-approval should be prevented\\n');
    } catch (error) {
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Should prevent large cash payments\\n');
    } catch (error) {
      console.log(`   ✅ VALIDATION WORKING: ${error}`);
      console.log('   🛡️ Cash payments limited to ₦100,000 for audit compliance\\n');
    }
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Should require additional docs for high-value\\n');
    } catch (error) {
      console.log(`   ✅ VALIDATION WORKING: ${error}`);
      console.log('   🛡️ High-value expenses require detailed documentation\\n');
    }
    ...
    try {
      const highValueExpense = await expenseService.createExpense({
        ...
        purpose: 'Emergency power backup system for uninterrupted school operations during outages',
        paymentMethod: 'bank_transfer',
        paymentDate: '2024-12-01',
      ...
      console.log(`   ✅ SUCCESS: High-value expense created: ${highValueExpense.reference}`);
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Should prevent duplicate references\\n');
    } catch (error) {
      console.log(`   ✅ VALIDATION WORKING: ${error}`);
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Should require detailed equipment documentation\\n');
    } catch (error) {
      console.log(`   ✅ VALIDATION WORKING: ${error}`);
      console.log('   🛡️ Equipment purchases over ₦50,000 require detailed documentation\\n');
    }
    ...
    try {
      const equipmentExpense = await expenseService.createExpense({
        ...
        purpose: 'Administrative laptop for school records management and student data processing',
        paymentMethod: 'bank_transfer',
        paymentDate: '2024-12-01',
      ...
      console.log(`   ✅ SUCCESS: Equipment expense created: ${equipmentExpense.reference}`);
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Should require weekend justification\\n');
    } catch (error) {
      console.log(`   ✅ VALIDATION WORKING: ${error}`);
      console.log('   🛡️ Weekend expenses over ₦50,000 require detailed justification\\n');
    }
    ...
    try {
      ...
      console.log('   ❌ VALIDATION FAILED: Should prevent far future dates\\n');
    } catch (error) {
      console.log(`   ✅ VALIDATION WORKING: ${error}`);
      console.log('   🛡️ Payment dates cannot be more than 7 days in the future\\n');
    }
    ...
    console.log(`   📝 Created expense: ${workflowExpense.reference} [Status: ${workflowExpense.status}]`);

    // Approve the expense
    const approvedExpense = await expenseService.approveExpense(workflowExpense.id, 'admin-456');
    console.log(`   ✅ Approved expense: ${approvedExpense.reference} [Status: ${approvedExpense.status}]`);
    console.log(`   👤 Approved by: ${approvedExpense.approvedBy}`);

    // Mark as paid
    const paidExpense = await expenseService.markAsPaid(approvedExpense.id);
    console.log(`   💰 Marked as paid: ${paidExpense.reference} [Status: ${paidExpense.status}]\\n`);
    ...
    console.log(`   ❌ Rejected expense: ${rejectedExpense.reference} [Status: ${rejectedExpense.status}]`);
    ...
    console.log('💪 Al-Muhaasib is protected by comprehensive business rule enforcement! 🛡️✨');
