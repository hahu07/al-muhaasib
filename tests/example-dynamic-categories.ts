...
async function demonstrateDynamicCategories() {
  ...
  try {
    ...
    const marketingCategory = await expenseCategoryService.createCategory({
      ...
      description: 'Online advertising, social media marketing, and digital campaigns',
      budgetCode: 'MKT-001'
    });
    console.log(`✅ Created: ${marketingCategory.name} (${marketingCategory.category})`);
    ...
    const profDevCategory = await expenseCategoryService.createCategory({
      ...
      description: 'Conference attendance, certification fees, and training programs'
    });
    console.log(`✅ Created: ${profDevCategory.name} (${profDevCategory.category})`);
    ...
    console.log(`✅ Created: ${eventCategory.name} (${eventCategory.category})\n`);
    ...
    const groupedCategories = categories.reduce((acc, cat) => {
      const predefinedCategories = expenseCategoryService.getPredefinedCategories().map(c => c.value);
      const type = predefinedCategories.includes(cat.category) ? 'Predefined' : 'Custom';
    ...
    Object.entries(groupedCategories).forEach(([type, cats]) => {
      ...
      cats.forEach(cat => {
        console.log(`  • ${cat.name} - ${cat.category} ${cat.budgetCode ? `[${cat.budgetCode}]` : ''}`);
      });
    });
    ...
    if (utilitiesCategory) {
      ...
      console.log(`💰 Created expense: ${expenseA.description} - ₦${expenseA.amount.toLocaleString()}`);
    }
    ...
    console.log(`💰 Created expense: ${marketingExpense.description} - ₦${marketingExpense.amount.toLocaleString()}`);

    // 5. Demonstrate category management
    console.log('\n5. Category management operations...');

    // Update a category
    const updatedCategory = await expenseCategoryService.updateCategory(eventCategory.id, {
      description: 'Updated: All school events including graduations, sports, and cultural activities',
      budgetCode: 'EVT-002'
    });
    console.log(`🔄 Updated category: ${updatedCategory.name} - New budget code: ${updatedCategory.budgetCode}`);
    ...
    Object.entries(summary.byCategory).forEach(([category, data]) => {
      console.log(`  • ${category}: ₦${data.amount.toLocaleString()} (${data.count} expense${data.count !== 1 ? 's' : ''})`);
    });
