...
export function useRealtimeReports(
  ...
  const refreshReport = useCallback(
    async (reportType: "income" | "balance" | "cashflow" | "all") => {
      if (!navigator.onLine) {
...
export function useReportsSummary(data: RealtimeReportsData) {
  return {
    ...
    profitMargin:
      data.incomeStatement && data.incomeStatement.totalRevenue > 0
        ? (data.incomeStatement.netIncome / data.incomeStatement.totalRevenue) *
          100
        : 0,
    debtToEquityRatio:
      data.balanceSheet && data.balanceSheet.equity.totalEquity > 0
        ? data.balanceSheet.liabilities.totalLiabilities /
          data.balanceSheet.equity.totalEquity
        : 0,
  };
}
