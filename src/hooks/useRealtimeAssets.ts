...
export function useRealtimeAssets(): UseRealtimeAssetsReturn {
  ...
  const calculateStatistics = useCallback((assetList: SimpleAsset[]): AssetStatistics => {
    const now = new Date();
    const thirtyDaysFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);

    // Count by status
    const activeAssets = assetList.filter(a => a.status === 'active').length;
    const inactiveAssets = assetList.filter(a => a.status === 'inactive').length;
    const underMaintenance = assetList.filter(a => a.status === 'under_maintenance').length;
    const disposedAssets = assetList.filter(a => a.status === 'disposed').length;
    ...
    assetList.forEach(asset => {
      ...
      byCategory[asset.category].value += asset.currentValue || asset.purchasePrice;
    });
    ...
    const needsMaintenance = assetList.filter(
      asset => asset.condition === 'poor' || asset.condition === 'needs_repair'
    );
