...
export function mapServiceCategoryToUI(category: AssetCategory): SimpleAsset['category'] {
...
export function mapUICategoryToService(category: SimpleAsset['category']): AssetCategory {
...
export function mapServiceStatusToUI(status: FixedAsset['status']): SimpleAsset['status'] {
...
export function mapUIStatusToService(status: SimpleAsset['status']): FixedAsset['status'] {
...
export function convertUIAssetToServiceCreation(asset: Omit<SimpleAsset, 'id' | 'createdAt' | 'updatedAt'>): Omit<FixedAsset, 'id' | 'createdAt' | 'updatedAt' | 'assetCode' | 'currentValue' | 'accumulatedDepreciation'> {
  return {
    ...
    warranty: asset.warranty ? {
      startDate: asset.warranty.startDate,
      endDate: asset.warranty.endDate,
      warrantyPeriodMonths: asset.warranty.endDate && asset.warranty.startDate ?
        Math.floor((new Date(asset.warranty.endDate).getTime() - new Date(asset.warranty.startDate).getTime()) / (1000 * 60 * 60 * 24 * 30)) : 0,
