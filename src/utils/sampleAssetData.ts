...
export const sampleAssets: Omit<
  FixedAsset,
  | "id"
  | "createdAt"
  | "updatedAt"
...
export async function createSampleAssets(): Promise<void> {
  ...
  try {
    ...
    if (existingAssets.length > 0) {
      console.log(
        `Found ${existingAssets.length} existing assets. Skipping sample data creation.`,
      );
      return;
    }
    ...
    for (const assetData of sampleAssets) {
      try {
        ...
        console.log(
          `✅ Created asset: ${asset.assetName} (${asset.assetCode})`,
        );
      } catch (error) {
        console.error(
          `❌ Failed to create asset: ${assetData.assetName}`,
          error,
        );
      }
    }
    ...
    console.log(
      `🎉 Successfully created ${createdAssets.length} sample assets!`,
    );

    // Calculate total value
    const totalValue = createdAssets.reduce(
