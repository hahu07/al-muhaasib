...
export class ClassService extends BaseDataService<SchoolClass> {
  ...
  async getClassesWithCapacity(): Promise<SchoolClass[]> {
    ...
    return classes.filter(c => 
      c.isActive && c.capacity && c.currentEnrollment < c.capacity
    );
  }
   ...
   */
  async updateEnrollment(classId: string, change: number): Promise<SchoolClass> {
   ...
   */
  async getByNameAndSection(name: string, section?: string, academicYear?: string): Promise<SchoolClass | null> {
    const classes = await this.list();
  ...
  async getEnrollmentStats(): Promise<{
    ...
    byLevel: Record<string, { classes: number; enrollment: number; capacity: number }>;
  }> {
    ...
    const byLevel: Record<string, { classes: number; enrollment: number; capacity: number }> = {};
