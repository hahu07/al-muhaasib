...
export default function AssetList({ onEditAsset, onViewAsset, refreshTrigger, preloadedAssets }: AssetListProps) {
  ...
  const handleStatusChange = async (asset: SimpleAsset, newStatus: SimpleAsset['status']) => {
    try {
      // Map UI status to service layer status if needed
      const serviceStatus = newStatus === 'under_maintenance' ? 'under-maintenance' : newStatus;
  ...
  const getStatusBadge = (status: SimpleAsset['status']) => {
    const statusConfig = {
      active: { variant: 'default' as const, icon: CheckCircle, color: 'bg-green-100 text-green-800' },
      inactive: { variant: 'secondary' as const, icon: XCircle, color: 'bg-gray-100 text-gray-800' },
      disposed: { variant: 'destructive' as const, icon: Trash2, color: 'bg-red-100 text-red-800' },
      under_maintenance: { variant: 'outline' as const, icon: AlertTriangle, color: 'bg-yellow-100 text-yellow-800' }
    };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">
                {formatCurrency(assets.reduce((sum, asset) => sum + (asset.currentValue || asset.purchasePrice), 0))}
              </p>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border overflow-hidden">
        ...
        {filteredAssets.length === 0 ? (
          <div className="p-8 text-center">
            ...
            {assets.length === 0 && !searchTerm && selectedCategory === 'all' && selectedStatus === 'all' && selectedCondition === 'all' && (
              <Button 
                onClick={handleCreateSampleData} 
                disabled={creatingData}
                className="mt-2"
              >
                {creatingData ? (
                  <><RefreshCw className="w-4 h-4 mr-2 animate-spin" /> Creating Sample Assets...</>
                ) : (
                  <>+ Create Sample Assets</>
                )}
              </Button>
                    ...
                    <td className="px-4 py-4">
                      <div className="space-y-1">
                        <div className="text-sm text-gray-900 dark:text-gray-100">
                          {assetCategories.find(c => c.value === asset.category)?.label || asset.category}
                        </div>
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button 
                            ...
                            title="Asset Actions"
                            onClick={() => console.log('Actions button clicked for asset:', asset.name)}
                          >
                        ...
                        <DropdownMenuContent align="end" className="bg-white border border-gray-300 shadow-lg">
                          ...
                          {asset.status === 'active' ? (
                            <DropdownMenuItem 
                              onClick={() => handleStatusChange(asset, 'inactive')}
