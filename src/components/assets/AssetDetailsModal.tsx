...
export default function AssetDetailsModal({
  ...
  const isWarrantyActive = asset.warranty && 
  ...
  const isWarrantyExpiring = asset.warranty && 
    asset.warranty.endDate && 
    new Date(asset.warranty.endDate) > new Date() &&
    new Date(asset.warranty.endDate) <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-lg">{assetCategories[asset.category] || asset.category}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Serial Number</p>
                  <p className="text-lg">{asset.serialNumber || 'Not specified'}</p>
                </div>
              </div>
              <div className="space-y-3">
                <div>
            ...
            {asset.currentValue && asset.currentValue !== asset.purchasePrice && (
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <p className="text-sm text-blue-800 dark:text-blue-200">
                  <strong>Depreciation:</strong> {formatCurrency(asset.purchasePrice - asset.currentValue)} 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-lg">{asset.warranty.provider || 'Not specified'}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Terms</p>
                    <p className="text-gray-900 dark:text-gray-100">{asset.warranty.terms || 'Not specified'}</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-lg">
                      {asset.warranty.startDate ? formatDate(asset.warranty.startDate) : 'Not specified'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">End Date</p>
                    <p className="text-lg">
                      {asset.warranty.endDate ? formatDate(asset.warranty.endDate) : 'Not specified'}
                    </p>
                  </div>
                </div>
              </div>
