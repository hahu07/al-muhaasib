...
export const ExpenseApprovalDashboard: React.FC<ExpenseApprovalDashboardProps> = ({
  className = '',
}) => {
  ...
  const loadData = async () => {
    try {
      ...
      const [pending, approved] = await Promise.all([
        expenseService.getByStatus('pending'),
        expenseService.getByStatus('approved'),
      ]);
      ...
      const totalPendingValue = pending.reduce((sum, exp) => sum + exp.amount, 0);
      const totalApprovedValue = approved.reduce((sum, exp) => sum + exp.amount, 0);
  ...
  const handleReject = async (expenseId: string) => {
    ...
    try {
      setProcessingIds(prev => new Set(prev).add(expenseId));
      await expenseService.rejectExpense(expenseId, reason.trim());
      await loadData(); // Reload data
    } catch (error) {
  ...
  const handleBulkApprove = async () => {
    if (!appUser || !confirm(`Approve all ${getFilteredPendingExpenses().length} visible expenses?`)) return;
  ...
  const getFilteredPendingExpenses = () => {
    ...
    // Amount filter
    if (filterAmount !== 'all') {
      filtered = filtered.filter(exp => getAmountCategory(exp.amount) === filterAmount);
    }
  ...
  const uniqueCategories = [...new Set(pendingExpenses.map(exp => exp.category))];
  ...
  return (
    <div className={`space-y-6 ${className}`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.totalPending}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.totalApproved}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.urgentCount}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                {stats.totalPending > 0 ? formatCurrency(stats.totalPendingValue / stats.totalPending) : '₦0'}
              </p>
            </div>
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
        ...
        <select
          value={filterAmount}
          onChange={(e) => setFilterAmount(e.target.value as 'all' | 'low' | 'medium' | 'high')}
        ...
        {filteredPending.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {filteredPending.map((expense) => (
              <div key={expense.id} className="p-6 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-semibold ${getAmountColor(expense.amount)}`}>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900 dark:text-gray-100">{selectedExpense.reference}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900 dark:text-gray-100">{selectedExpense.categoryName}</p>
              </div>
              <div>
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900 dark:text-gray-100">{selectedExpense.description}</p>
            </div>

            {selectedExpense.purpose && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Purpose</label>
                <p className="mt-1 text-sm text-gray-900 dark:text-gray-100">{selectedExpense.purpose}</p>
              </div>
            )}
            ...
            {selectedExpense.notes && (
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900 dark:text-gray-100">{selectedExpense.notes}</p>
              </div>
            )}
