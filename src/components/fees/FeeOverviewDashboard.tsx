...
export function FeeOverviewDashboard() {
  ...
  const fetchOverviewData = async () => {
    try {
      ...
      const currentStructures = structures.filter(s => s.academicYear === academicYear);
      const currentAssignments = assignments.filter(a => a.academicYear === academicYear && a.term === selectedTerm);
      ...
      const overviewStats: OverviewStats = {
        ...
        totalRevenueAssigned: currentAssignments.reduce((sum, a) => sum + a.totalAmount, 0),
        totalRevenuePaid: currentAssignments.reduce((sum, a) => sum + a.amountPaid, 0),
        totalRevenueBalance: currentAssignments.reduce((sum, a) => sum + a.balance, 0),
        paymentSummary: {
          paidCount: currentAssignments.filter(a => a.status === 'paid').length,
          partialCount: currentAssignments.filter(a => a.status === 'partial').length,
          unpaidCount: currentAssignments.filter(a => a.status === 'unpaid').length,
        },
      };

      // Generate class summaries
      const summaries: ClassSummary[] = classes.map(cls => {
        const classStructure = currentStructures.find(s => 
          s.classId === cls.id && s.term === selectedTerm && s.isActive
        );
        const classAssignments = currentAssignments.filter(a => a.classId === cls.id);
        ...
        return {
          ...
          totalAmount: classAssignments.reduce((sum, a) => sum + a.totalAmount, 0),
          paidAmount: classAssignments.reduce((sum, a) => sum + a.amountPaid, 0),
          balance: classAssignments.reduce((sum, a) => sum + a.balance, 0),
          paidStudents: classAssignments.filter(a => a.status === 'paid').length,
          partialStudents: classAssignments.filter(a => a.status === 'partial').length,
          unpaidStudents: classAssignments.filter(a => a.status === 'unpaid').length,
        };
      });
    ...
    return (
      <div className="space-y-6">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-64 bg-gray-100 dark:bg-gray-700/50 rounded-lg"></div>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <select
            value={selectedTerm}
            onChange={(e) => setSelectedTerm(e.target.value as 'first' | 'second' | 'third')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          icon={<DollarSignIcon className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
          color="blue"
        ...
        <StatCard
          ...
          icon={<BookOpenIcon className="w-5 h-5 text-purple-600 dark:text-purple-400" />}
          color="purple"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Partial Payment</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">No Payment</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">
            Class-wise Fee Summary ({selectedTerm.charAt(0).toUpperCase() + selectedTerm.slice(1)} Term)
          </h3>
        </div>
        ...
        {classSummaries.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400">No class data available</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {classSummaries.map((summary) => (
                  <tr key={summary.classId} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex gap-1 text-xs">
                        {summary.paidStudents > 0 && (
                          <span className="inline-flex px-2 py-1 rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">
                            {summary.paidStudents} paid
                          </span>
                        )}
                        {summary.partialStudents > 0 && (
                          <span className="inline-flex px-2 py-1 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                            {summary.partialStudents} partial
                          </span>
                        )}
                        {summary.unpaidStudents > 0 && (
                          <span className="inline-flex px-2 py-1 rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">
