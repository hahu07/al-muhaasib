...
import { Plus, Trash2, User, Phone, Mail, Building2, Banknote, Calendar, Hash } from 'lucide-react';
...
export default function StaffForm({ staff, onSuccess, onCancel }: StaffFormProps) {
  ...
  const validateForm = () => {
    ...
    // Validation rules
    if (formData.surname.length > 50) newErrors.surname = 'Surname cannot exceed 50 characters';
    if (formData.firstname.length > 50) newErrors.firstname = 'First name cannot exceed 50 characters';
    ...
    if (formData.staffNumber.length < 3 || formData.staffNumber.length > 20) {
      newErrors.staffNumber = 'Staff number must be between 3 and 20 characters';
    }
    ...
    if (formData.basicSalary < 30000) {
      newErrors.basicSalary = 'Basic salary cannot be below minimum wage (₦30,000)';
    }
    ...
    if (formData.employmentType === 'contract' && formData.basicSalary > 5000000) {
      newErrors.basicSalary = 'Contract staff salary cannot exceed ₦5,000,000';
    }
    if (formData.employmentType === 'part-time' && formData.basicSalary > 3000000) {
    ...
    if (hasBankName && !hasAccountNumber) {
      newErrors.accountNumber = 'Account number required when bank name is provided';
    }
    ...
    if (formData.basicSalary > 5000000 && (!hasBankName || !hasAccountNumber)) {
      newErrors.bankName = 'Banking details required for salaries over ₦5,000,000';
    }
    ...
    allowances.forEach((allowance, index) => {
      ...
      if (allowance.amount > 1000000) {
        newErrors[`allowance_${index}_amount`] = 'Allowance amount cannot exceed ₦1,000,000';
      }
    });
    ...
    if (totalAllowances > formData.basicSalary * 2) {
      newErrors.allowances = 'Total allowances cannot exceed 200% of basic salary';
    }
  ...
  const addAllowance = () => {
    ...
    setAllowances(prev => [...prev, { name: '', amount: 0, isRecurring: true }]);
  };
  ...
  const updateAllowance = (index: number, field: keyof AllowanceForm, value: string | number | boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const staffData = {
        ...formData,
        allowances: allowances.length > 0 ? allowances as StaffAllowance[] : undefined,
      };
  ...
  const totalCompensation = formData.basicSalary + allowances.reduce((sum, a) => sum + (a.amount || 0), 0);
            ...
            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  ...
                  />
                  {errors.staffNumber && <p className="text-sm text-red-500 mt-1">{errors.staffNumber}</p>}
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleInputChange('employmentType', value as typeof formData.employmentType)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
                  ...
                  />
                  {errors.employmentDate && <p className="text-sm text-red-500 mt-1">{errors.employmentDate}</p>}
                </div>
                
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    <Label htmlFor="isActive">Active Status</Label>
                    <p className="text-sm text-gray-500">Staff member is currently active</p>
                  </div>
                  <Switch
                    id="isActive"
            ...
            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {errors.basicSalary && <p className="text-sm text-red-500 mt-1">{errors.basicSalary}</p>}
                </div>
                
                <div className="flex items-center justify-center p-4 bg-gray-50 rounded-lg">
                  <div className="text-center">
                    <p className="text-sm text-gray-600">Total Monthly Compensation</p>
            ...
            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
              ...
              {errors.allowances && (
                <div className="bg-red-50 border border-red-200 rounded-md p-3 mb-4">
                  <p className="text-red-600 text-sm">{errors.allowances}</p>
                </div>
              )}
              ...
              <div className="space-y-3">
                  ...
                  <div key={index} className="border rounded-lg p-4">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      <div className="md:col-span-2">
                        <Label htmlFor={`allowance_${index}_name`}>Allowance Name</Label>
                        <Input
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={(checked) => updateAllowance(index, 'isRecurring', checked)}
                          />
                          <Label htmlFor={`allowance_${index}_recurring`} className="text-sm">
                            Recurring
                          </Label>
                        </div>
                        <Button
                ...
                {allowances.length === 0 && (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm">Click &quot;Add Allowance&quot; to add monthly allowances</p>
                  </div>
                )}
              </div>
            </div>
