...
import { StudentProfile as StudentProfileType, Payment, StudentFeeAssignment } from '@/types';
import { studentService, paymentService, studentFeeAssignmentService } from '@/services';
...
export default function StudentProfile({ studentId, studentData }: StudentProfileProps) {
  ...
  const handleEditSave = async () => {
    ...
    try {
      setLoading(true);
      const updatedStudent = await studentService.update(student.id, editFormData);
  ...
  const renderEditableField = (
    ...
    return (
      <div>
        <label className="text-sm font-medium text-gray-500 dark:text-blue-400">{label}</label>
        {isEditing ? (
          type === 'select' ? (
            <Select 
              value={editValue || ''} 
          ...
          <p className="text-gray-900 dark:text-blue-100 font-medium">
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-blue-300">Loading student profile...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              {isEditing ? (
                ...
                <h1 className="text-3xl font-bold text-gray-900 dark:text-blue-100 mb-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-bold text-gray-900 dark:text-blue-100 mb-4 flex items-center gap-2">
                <User className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                Student Details
              </h2>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium text-gray-500 dark:text-blue-400">Class</label>
                  <p className="text-gray-900 dark:text-blue-100 font-medium">{student.className}</p>
                </div>
                {renderEditableField('Gender', 'gender', student.gender, 'select', [
                  { value: '', label: 'Not specified' },
                  { value: 'male', label: 'Male' },
                  { value: 'female', label: 'Female' }
                ])}
                {renderEditableField('Date of Birth', 'dateOfBirth', student.dateOfBirth, 'date')}
                <div>
                  <label className="text-sm font-medium text-gray-500 dark:text-blue-400">Admission Date</label>
                  <p className="text-gray-900 dark:text-blue-100">
                    {student.admissionDate ? formatDate(student.admissionDate) : 'Not specified'}
                  </p>
                </div>
                {renderEditableField('Blood Group', 'bloodGroup', student.bloodGroup)}
                <div>
                  <label className="text-sm font-medium text-gray-500 dark:text-blue-400">Status</label>
                  <p className="text-gray-900 dark:text-blue-100 capitalize">{student.isActive ? 'Active' : 'Inactive'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-4">
                {isEditing ? (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <Input
                        ...
                        onChange={(e) => handleEditChange('guardianFirstname', e.target.value)}
                      />
                      <Input
                        label="Guardian Surname"
                        value={editFormData.guardianSurname || ''}
                        onChange={(e) => handleEditChange('guardianSurname', e.target.value)}
                      />
                    </div>
                    <Input
                      ...
                      onChange={(e) => handleEditChange('guardianPhone', e.target.value)}
                    />
                    <Input
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleEditChange('guardianAddress', e.target.value)}
                    ...
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => handleEditChange('guardianRelationship', value)}
                      >
                        <SelectTrigger>
                  ...
                  <>
                    <div>
                      <label className="text-sm font-medium text-gray-500 dark:text-blue-400">Name</label>
                      <p className="text-gray-900 dark:text-blue-100 font-medium">
                        {student.guardianSurname} {student.guardianFirstname}
                      </p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-500 dark:text-blue-400 flex items-center gap-1">
                        <Phone className="w-3 h-3" />
                        Phone
                      </label>
                      <p className="text-gray-900 dark:text-blue-100">{student.guardianPhone}</p>
                    </div>
                    {student.guardianEmail && (
                      <div>
                        ...
                        <p className="text-gray-900 dark:text-blue-100 break-words">{student.guardianEmail}</p>
                      </div>
                    )}
                    {student.guardianAddress && (
                      <div>
                        ...
                        <p className="text-gray-900 dark:text-blue-100">{student.guardianAddress}</p>
                      </div>
                    )}
                    <div>
                      <label className="text-sm font-medium text-gray-500 dark:text-blue-400">Relationship</label>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl shadow-lg p-6 text-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                  <p className="text-sm text-blue-100 mb-1">Total Fees</p>
                  <p className="text-2xl font-bold">{formatCurrency(student.totalFeesAssigned)}</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                  <p className="text-sm text-blue-100 mb-1">Amount Paid</p>
                  <p className="text-2xl font-bold">{formatCurrency(student.totalPaid)}</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
              ...
              {feeAssignments.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-700/50">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-blue-300 uppercase tracking-wider">
                          Fee Type
                        </th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-gray-700 dark:text-blue-300 uppercase tracking-wider">
                          Amount
                        </th>
                        <th className="px-4 py-3 text-center text-xs font-medium text-gray-700 dark:text-blue-300 uppercase tracking-wider">
                          Status
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      {feeAssignments.map((assignment) => (
                        <tr key={assignment.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          ...
                          <td className="px-4 py-3 text-right text-gray-900 dark:text-blue-100 font-medium">
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 dark:text-blue-300">No fee assignments yet</p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
              ...
              {payments.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      {payments.map((payment) => (
                        <tr key={payment.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          <td className="px-4 py-3 text-gray-900 dark:text-blue-100 text-sm">
                            {formatDate(payment.paymentDate)}
                          </td>
                          <td className="px-4 py-3 text-gray-600 dark:text-blue-300 text-sm font-mono">
                            {payment.reference}
                          </td>
                          <td className="px-4 py-3 text-right text-gray-900 dark:text-blue-100 font-semibold">
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 dark:text-blue-300">No payments recorded yet</p>
                  <p className="text-sm text-gray-500 dark:text-blue-400 mt-1">
