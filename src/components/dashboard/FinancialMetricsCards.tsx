...
function MetricCard({ 
  ...
  const TrendIcon = changeType === 'increase' ? TrendingUpIcon : 
                   changeType === 'decrease' ? TrendingDownIcon : null;

  return (
    <div className={`
      relative p-6 bg-white rounded-xl border-2 shadow-sm hover:shadow-md transition-shadow
      ${alert ? 'ring-2 ring-red-200 border-red-300' : 'border-gray-200'}
    `}>
      {alert && (
...
export function FinancialMetricsCards({ data }: FinancialMetricsCardsProps) {
  ...
  const metrics = [
    ...
    },
    {
      ...
      changeType: data.profitLoss.netProfit >= 0 ? ('increase' as const) : ('decrease' as const),
      icon: <TrendingUpIcon className="w-6 h-6" />,
      description: `${formatPercentage(data.profitLoss.profitMargin)} profit margin`,
      color: data.profitLoss.netProfit >= 0 ? ('green' as const) : ('red' as const),
    },
    {
      title: 'Cash Balance',
