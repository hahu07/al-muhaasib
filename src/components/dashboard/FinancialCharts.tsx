...
export function ExpenseBreakdownChart({ data }: ExpenseBreakdownChartProps) {
  ...
  const chartData = Object.entries(data.expenses.byCategory).map(
    ([category, expense]) => ({
      label: category
        .replace(/_/g, " ")
      ...
      color:
        categoryColors[category as keyof typeof categoryColors] || "#64748b",
    }),
  );
...
export function AccountBalancesChart({ data }: AccountBalancesChartProps) {
  ...
  const chartData = Object.entries(data.accounts.byType).map(
    ([type, balance]) => ({
...
export function StudentPaymentStatusChart({
  data,
}: StudentPaymentStatusChartProps) {
  const statusData = [
    {
      label: "Paid",
  ...
  return (
    <PieChart
      ...
      total={data.students.totalStudents}
    />
  );
}
