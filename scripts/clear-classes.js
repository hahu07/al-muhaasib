...
async function clearAllClasses() {
  try {
    ...
    classes.forEach((cls, index) => {
      const data = cls.data;
      console.log(`  ${index + 1}. ${data.name}${data.section ? ` ${data.section}` : ''} (${data.level})`);
    });
    ...
    for (let i = 0; i < classes.length; i++) {
      ...
      try {
        ...
        console.log(`✅ Deleted: ${data.name}${data.section ? ` ${data.section}` : ''}`);
      } catch (error) {
        console.error(`❌ Failed to delete ${data.name}${data.section ? ` ${data.section}` : ''}:`, error.message);
      }
    }
    
    console.log(`\n🎉 Successfully deleted ${classes.length} classes!`);
    console.log('💡 You can now create fresh classes using the Class Management interface.');
